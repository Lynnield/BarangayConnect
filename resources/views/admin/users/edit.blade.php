...
<div class="w-full max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-white tracking-tight">Edit Account: {{ $user->name }}</h1>
                <p class="text-sm text-slate-500 font-medium mt-1">Modify user privileges and system access status.</p>
            </div>
        </div>
        <div class="hidden md:block">
            <x-badge type="{{ $user->status === 'active' ? 'success' : ($user->status === 'suspended' ? 'danger' : 'neutral') }}">
                {{ $user->status }}
            </x-badge>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="p-8 space-y-8">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">New Password (Optional)</label>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="password" name="password_confirmation"
                            class="block w-full rounded-2xl border border-slate-700 bg-slate-800/50 py-3 px-4 text-xs text-white placeholder:text-slate-600 focus:border-indigo-500 focus:bg-slate-800 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
                            placeholder="Confirm only if changing">
                    </div>
                </div>
            </div>
