...
@section('breadcrumb')
    <a href="{{ route('staff.residents.index') }}" class="text-slate-500 hover:text-indigo-400 transition-colors">Residents</a>
    ...
    <x-card class="border-none shadow-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-indigo-950" :padding="false">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between p-8 gap-8">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <x-button href="{{ route('staff.residents.edit', $resident) }}" variant="primary" size="md" icon="edit-3" class="shadow-indigo-600/20">
                    Edit Record
                </x-button>
                @if($resident->verification_status !== 'verified')
                    <form method="POST" action="{{ route('staff.residents.verify', $resident) }}">
                        @csrf
                        <x-button type="submit" variant="secondary" size="md" icon="shield-check">
                            Approve
                        </x-button>
                    </form>
                @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        @forelse($resident->documentRequests as $d)
                            <tr class="hover:bg-slate-800/30 transition-all group">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('staff.requests.show', $d) }}" class="text-indigo-400 hover:text-indigo-300 transition-colors">
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            @if($resident->verification_status !== 'rejected')
                <x-card title="Reject Verification" icon="shield-x" class="bg-slate-900/50 border-slate-800">
                    <form method="POST" action="{{ route('staff.residents.reject-verification', $resident) }}" class="space-y-4">
                        @csrf
                        <textarea name="verification_notes" rows="4" class="block w-full rounded-2xl border border-slate-700 bg-slate-800/50 py-3 px-4 text-xs text-white focus:border-indigo-500 focus:bg-slate-800 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none resize-none" placeholder="Reason or missing document"></textarea>
                        <x-button type="submit" variant="danger" size="sm" icon="x-circle" class="w-full">
                            Reject Verification
                        </x-button>
                    </form>
                </x-card>
            ...
            <x-card class="bg-indigo-600/5 border-indigo-500/20">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h4 class="text-xs font-black text-white uppercase tracking-widest">Staff Note</h4>
                        <p class="text-[11px] text-slate-400 mt-1 leading-relaxed">
                            Resident records are verified upon registration. Any changes to legal information require administrative approval.
                        </p>
                    </div>
                </div>
            </x-card>
